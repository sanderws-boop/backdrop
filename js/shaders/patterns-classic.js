(function() {
    ...
    Studio.Shaders.PatternsClassic = [
        ...
        },
        {
            name: "Color Mesh",
            category: "organic",
            frag: "void main(){\n  vec2 uv=gl_FragCoord.xy/u_resolution;\n  vec2 p=uv;\n  float asp=u_resolution.x/u_resolution.y;\n  p.x*=asp;p=rotateP(p,asp);\n  float t=u_seed+organicTime(u_time,0.06,0.2);\n  vec2 q=p*0.25;\n  vec2 w1=vec2(snoise(vec3(q+1.7,t*0.1)),snoise(vec3(q+4.3,t*0.08)))*u_warp*0.45;\n  vec2 w2=vec2(snoise(vec3((q+w1*0.5)*0.7+9.0,t*0.06)),snoise(vec3((q+w1*0.5)*0.7+14.0,t*0.05)))*u_warp*0.25;\n  vec2 pw=p+w1+w2;\n  vec2 nq=pw*u_scale*0.55;\n  vec3 blended=vec3(0.0);\n  float totalW=0.0;\n  for(int i=0;i<5;i++){\n    if(i>=u_numColors) break;\n    float fi=float(i);\n    float n=snoise(vec3(nq+fi*7.31,t*0.08+fi*3.17));\n    n+=snoise(vec3(nq*1.8+fi*13.7,t*0.12+fi*5.31))*0.45;\n    n+=snoise(vec3(nq*3.5+fi*21.3,t*0.15+fi*8.7))*0.15;\n    float w=exp(n*12.0);\n    blended+=linearToOklab(srgbToLinear(getColor(i)))*w;\n    totalW+=w;\n  }\n  blended/=max(totalW,0.001);\n  vec3 col=oklabToLinear(blended);\n  col=max(col,vec3(0.0));\n  outputColorDirect(col,uv);\n}"
        },
    ];
})();
