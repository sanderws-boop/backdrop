(function() {
    ...
    Studio.Shaders.PatternsClassic = [
        ...
        },
        {
            name: "Color Mesh",
            category: "organic",
            frag: "void main(){\n  vec2 uv=gl_FragCoord.xy/u_resolution;\n  vec2 p=uv;\n  float asp=u_resolution.x/u_resolution.y;\n  p.x*=asp;p=rotateP(p,asp);\n  float t=u_seed+organicTime(u_time,0.06,0.2);\n  vec2 q=p*u_scale*0.15;\n  vec2 w1=vec2(snoise(vec3(q+1.7,t*0.1)),snoise(vec3(q+4.3,t*0.08)))*u_warp*0.5;\n  vec2 w2=vec2(snoise(vec3((q+w1*0.5)*0.7+9.0,t*0.06)),snoise(vec3((q+w1*0.5)*0.7+14.0,t*0.05)))*u_warp*0.3;\n  vec2 pw=p+w1+w2;\n  vec3 blended=vec3(0.0);\n  float totalW=0.0;\n  for(int i=0;i<5;i++){\n    if(i>=u_numColors) break;\n    float fi=float(i);\n    float theta=fi*2.39996323;\n    float r=0.32+fi*0.05;\n    vec2 anchor=vec2(0.5*asp+cos(theta)*r*asp*0.55,0.5+sin(theta)*r*0.55);\n    anchor.x+=sin(t*(0.07+fi*0.02)+fi*2.7)*0.18*asp;\n    anchor.y+=cos(t*(0.05+fi*0.015)+fi*1.9)*0.14;\n    float d=distance(pw,anchor);\n    float sigma=(0.5+fi*0.05)/u_scale;\n    float w=exp(-d*d/(2.0*sigma*sigma));\n    blended+=linearToOklab(srgbToLinear(getColor(i)))*w;\n    totalW+=w;\n  }\n  blended/=max(totalW,0.001);\n  vec3 col=oklabToLinear(blended);\n  col=max(col,vec3(0.0));\n  outputColorDirect(col,uv);\n}"
        },
    ];
})();
